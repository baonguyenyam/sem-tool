...
var LIFT_APP = {
  ...
  gen: function gen() {
    for (var t = 0; t < this.akw_get().length; t++) {
      var e = lift_encode(this.akw_get()[t].trim());
      if (0 < e.length) for (var _ = 0; _ < this.bkw_get().length; _++) {
        var i = lift_encode(this.bkw_get()[_].trim());

        if (0 < i.length) {
          this.KW.push(e + " " + i);

          for (var r = 0; r < this.ckw_get().length; r++) {
            var a = lift_encode(this.ckw_get()[r].trim());

            if (0 < a.length) {
              this.KW.push(e + " " + i + " " + a);

              for (var l = 0; l < this.dkw_get().length; l++) {
                var n = lift_encode(this.dkw_get()[l].trim());

                if (0 < n.length) {
                  this.KW.push(e + " " + i + " " + a + " " + n);

                  for (var o = 0; o < this.ekw_get().length; o++) {
                    var s = lift_encode(this.ekw_get()[o].trim());
                    0 < s.length && this.KW.push(e + " " + i + " " + a + " " + n + " " + s);
                  }
                }
              }
            }
  ...
  genmulti: function genmulti() {
    ...
    try {
      for (var i = 0; i < this.akw_get().length; i++) {
        var r = lift_encode(this.akw_get()[i].trim());
        if (0 < r.length) for (var a = 0; a < this.bkw_get().length; a++) {
          var l = lift_encode(this.bkw_get()[a].trim());

          if (0 < l.length) {
            e++, this.KW.push(r + " " + l), this.replaceMultiKW(this.rePlaceMulti, e, r, l);

            for (var n = 0; n < this.ckw_get().length; n++) {
              var o = lift_encode(this.ckw_get()[n].trim());

              if (0 < o.length) {
                e++, this.KW.push(r + " " + l + " " + o), this.replaceMultiKW(this.rePlaceMulti, e, r, l, o);

                for (var s = 0; s < this.dkw_get().length; s++) {
                  var c = lift_encode(this.dkw_get()[s].trim());

                  if (0 < c.length) {
                    e++, this.KW.push(r + " " + l + " " + o + " " + c), this.replaceMultiKW(this.rePlaceMulti, e, r, l, o, c);

                    for (var h = 0; h < this.ekw_get().length; h++) {
                      var g = lift_encode(this.ekw_get()[h].trim());
                      0 < g.length && (e++, this.KW.push(r + " " + l + " " + o + " " + c + " " + g), this.replaceMultiKW(this.rePlaceMulti, e, r, l, o, c, g));
                    }
                  }
                }
              }
      ...
      for (var u = t.replace("___LIFTCHANGE___", this.rePlaceMulti_Done.join("")).replace(/<category[^>]*>(.*?)<\/category>/gi, "___STATELOCATION___"), p = 0; p < this.getStateAndLocation.state.length; p++) {
        var d = this.getStateAndLocation.state[p].trim();
        _ += '<category domain="category" nicename="' + buildStringURL(this.getStateAndLocation.state[p]) + '"><![CDATA[' + d + "]]></category>";
      }

      for (var m = 0; m < this.getStateAndLocation.location.length; m++) {
        var w = this.getStateAndLocation.location[m].trim();
        _ += '<category domain="category" nicename="' + buildStringURL(this.getStateAndLocation.location[m]) + '"><![CDATA[' + w + "]]></category>";
      }

      u = u.replace(/___STATELOCATION___/gi, _).replace(/___LIFTCHANGE___/gi, ""), $("#resultsmulti").val(unReplaceLIFT(u)), $("#results").val(lift_decode(LIFT_APP.KW.join("\n"))), $("#number").text(this.rePlaceMulti_Done.length), $("#multiresults .uv").addClass("d-none"), $("#multiresults .rv").removeClass("d-none");
      var f = $("#resultsmulti").val(),
  ...
  replaceMultiKW: function replaceMultiKW(t, e, _, i, r, a, l) {
    var n = _ ? _ + " " : "",
        ...
        c = a ? a + " " : "",
        h = l ? l + " " : "",
        g = _ || i || r || a || l ? n + o + s + c + h : "",
        u = makeid(5),
        p = t.match(/<item>(.*?)<\/item>/gi).map(function (t) {
      return t;
    })[0].replace(/(___REPLACE___|___replace___)/gi, g.trim()).replace(/(___REPLACE_A___|___replace_a___)/gi, n.trim()).replace(/(___REPLACE_B___|___replace_b___)/gi, o.trim()).replace(/(___REPLACE_C___|___replace_c___)/gi, s.trim()).replace(/(___REPLACE_D___|___replace_d___)/gi, c.trim()).replace(/(___REPLACE_E___|___replace_e___)/gi, h.trim()).trim();
    this.rePlaceMulti_Done.push(p.replace(/<wp:post_id>(.*?)<\/wp:post_id>/gi, "<wp:post_id>" + u + "</wp:post_id>"));
  }
};
LIFT_APP.init();
"use strict";
...
$("#clear").on("click", function () {
  ...
  if ($("#source").val() || (e += "Please upload source file\n"), $("#keywordds").val() || (e += "Please enter keywords list\n"), e) alert(e);else {
    for (var t = $("#keywordds").val().split("\n"), a = "", l = [], r = replaceLIFT(LIFT_APP.code.getValue()), o = r.replace(/<item>(.*?)<\/item>/gi, "___LIFTCHANGE___"), _ = r.match(/<item>(.*?)<\/item>/gi).map(function (e) {
      return e;
    }), s = 0; s < _.length; s++) {
      a += "___LIFTCHANGE___";
    }
    ...
    for (var i = 0; i < t.length; i++) {
      var c = makeid(5);
      t[i] && 1 < t[i].length && l.push(_[0].replace(/(___REPLACE___|___replace___)/gi, t[i].trim()).replace(/<wp:post_id>(.*?)<\/wp:post_id>/gi, "<wp:post_id>" + c + "</wp:post_id>"));
    }

    var n = o.replace(a, l.join(""));
    $("#results").val(unReplaceLIFT(n));
    var p = $("#results").val(),
        m = "LIFT_KW_LIST_" + new Date().getTime() + ".xml";
    download(m, p);
  }
