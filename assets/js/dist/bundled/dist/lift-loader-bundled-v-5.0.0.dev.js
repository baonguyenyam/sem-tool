...
function lift_encode(e) {
  return unescape(encodeURIComponent(e));
}

function lift_decode(e) {
...
function makeid(e) {
  for (var t = [], n = "123456789", o = n.length, r = 0; r < e; r++) {
    t.push(n.charAt(Math.floor(Math.random() * o)));
  }

  return t.join("");
}

function replaceLIFT(e) {
  return e.toString().replace(/"/gi, "&#34;").replace(/'/gi, "&#39;").replace(/\n/gi, "").replace(/>\s+</g, "><").replace(/\n/g, "").replace(/[\t ]+\</g, "<").replace(/\>[\t ]+\</g, "><").replace(/\>[\t ]+$/g, ">");
}

function unReplaceLIFT(e) {
  return e.toString().replace(/&#34;/gi, '"').replace(/&#39;/gi, "'");
}

function resultHTMLValidate(e, t) {
  return '<div class="list-group-item d-flex justify-content-between"><strong>' + e + "</strong> " + t + "</div>";
}

function loadCSVFileAsText() {
  var e, t;
  document.getElementById("fileCSVToLoad").value ? (e = document.getElementById("fileCSVToLoad").files[0], t = new FileReader(), LIFT_APP.code && LIFT_APP.code.toTextArea(), t.onload = function (e) {
    var t = e.target.result;
    document.getElementById("csvsource").value = t, LIFT_APP.code = CodeMirror.fromTextArea(document.getElementById("csvsource"), {
      mode: "text/x-textile",
      lineNumbers: !0,
      styleActiveLine: !0,
      matchBrackets: !0,
    ...
    for (var n = LIFT_APP.code.getValue().replace(/^(?:\r\n?|\n|\r|\s*)/gm, "").replace(/\n*$/, "").split("\n")[0].split(","), o = "", r = 0; r < n.length; r++) {
      o += '<option value="' + r + '">' + n[r] + "</option>";
    }

    $("#readCSVTable").removeClass("d-none"), $("#readCSVTable #fkw").append(o), $("#readCSVTable #furl").append(o);
  }, t.readAsText(e, "UTF-8")) : alert("Please upload file");
}

function buildStringURL(e) {
  return e.toString().toLowerCase().trim().replace(/\s+/gi, "-").replace(/[\uE000-\uF8FF]/g, "").replace(/^[\u0080-\uffff]/g, "");
}

function loadFileAsText() {
  var e, t;
  document.getElementById("fileToLoad").value ? (e = document.getElementById("fileToLoad").files[0], t = new FileReader(), LIFT_APP.code && LIFT_APP.code.toTextArea(), t.onload = function (e) {
    var t = e.target.result;
    document.getElementById("source").value = t, localStorage.removeItem("myLIFT_Replace"), localStorage.setItem("myLIFT_Replace", t), LIFT_APP.code = CodeMirror.fromTextArea(document.getElementById("source"), {
      mode: "text/html",
      lineNumbers: !0,
      styleActiveLine: !0,
      matchBrackets: !0,
...
function extractHostname(e) {
  var t = -1 < e.indexOf("//") ? e.split("/")[2] : e.split("/")[0];
...
function extractRootDomain(e) {
  var t = extractHostname(e),
      n = t.split("."),
      o = n.length;
  return 2 < o && (t = n[o - 2] + "." + n[o - 1], 2 == n[o - 2].length && 2 == n[o - 1].length && (t = n[o - 3] + "." + t)), t;
}

function Validate() {
  var e = document.getElementById("txtPassword").value,
      t = document.getElementById("txtConfirmPassword").value;
  return e != t ? (alert("The password does not match"), !1) : !(e.length < 3 || t.length < 3) || (alert("Your Password is too short"), !1);
}

function generatePassword() {
  for (var e = "0123456789", t = "", n = 0, o = e.length; n < 6; ++n) {
    t += e.charAt(Math.floor(Math.random() * o));
  }
...
var LIFT_APP = {
  ...
  akw_get: function akw_get() {
    return this.__a_kw.val().trim().split(",");
  },
  bkw_get: function bkw_get() {
    return this.__b_kw.val().trim().split(",");
  },
  ckw_get: function ckw_get() {
    return this.__c_kw.val().trim().split(",");
  },
  dkw_get: function dkw_get() {
    return this.__d_kw.val().trim().split(",");
  },
  ekw_get: function ekw_get() {
    return this.__e_kw.val().trim().split(",");
  },
  gen: function gen() {
    for (var t = 0; t < this.akw_get().length; t++) {
      var e = this.akw_get()[t].trim();
      if (0 < e.length) for (var _ = 0; _ < this.bkw_get().length; _++) {
        var i = this.bkw_get()[_].trim();

        if (0 < i.length) {
          $("#l_b").prop("checked") || this.KW.push(e + " " + i);

          for (var r = 0; r < this.ckw_get().length; r++) {
            var a = this.ckw_get()[r].trim();

            if (0 < a.length) {
              $("#l_c").prop("checked") || this.KW.push(e + " " + i + " " + a);

              for (var l = 0; l < this.dkw_get().length; l++) {
                var c = this.dkw_get()[l].trim();

                if (0 < c.length) {
                  $("#l_d").prop("checked") || this.KW.push(e + " " + i + " " + a + " " + c);

                  for (var n = 0; n < this.ekw_get().length; n++) {
                    var o = this.ekw_get()[n].trim();
                    0 < o.length && ($("#l_e").prop("checked") || this.KW.push(e + " " + i + " " + a + " " + c + " " + o));
                  }
                }
              }
            }
    ...
    $("#results").text(LIFT_APP.KW.join("\n")), $("#boxresult .rs").removeClass("d-none"), $("#boxresult .rv").hide(), $("#number").text(LIFT_APP.KW.length);
  },
  genmulti: function genmulti() {
    this.rePlaceMulti = replaceLIFT(LIFT_APP.code.getValue()), this.getStateAndLocation.state = replaceLIFT($("#instate").val()).trim().split(","), this.getStateAndLocation.location = replaceLIFT($("#inlocation").val()).trim().split(",");
    var t = this.rePlaceMulti.replace(/<item>(.*?)<\/item>/gi, "___LIFTCHANGE___"),
        e = 0,
        _ = "";

    try {
      for (var i = 0; i < this.akw_get().length; i++) {
        var r = this.akw_get()[i].trim();
        if (0 < r.length) for (var a = 0; a < this.bkw_get().length; a++) {
          var l = this.bkw_get()[a].trim();

          if (0 < l.length) {
            $("#l_b").prop("checked") || (e++, this.KW.push(r + " " + l), this.replaceMultiKW(this.rePlaceMulti, e, r, l));

            for (var c = 0; c < this.ckw_get().length; c++) {
              var n = this.ckw_get()[c].trim();

              if (0 < n.length) {
                $("#l_c").prop("checked") || (e++, this.KW.push(r + " " + l + " " + n), this.replaceMultiKW(this.rePlaceMulti, e, r, l, n));

                for (var o = 0; o < this.dkw_get().length; o++) {
                  var s = this.dkw_get()[o].trim();

                  if (0 < s.length) {
                    $("#l_d").prop("checked") || (e++, this.KW.push(r + " " + l + " " + n + " " + s), this.replaceMultiKW(this.rePlaceMulti, e, r, l, n, s));

                    for (var h = 0; h < this.ekw_get().length; h++) {
                      var g = this.ekw_get()[h].trim();
                      0 < g.length && ($("#l_e").prop("checked") || (e++, this.KW.push(r + " " + l + " " + n + " " + s + " " + g), this.replaceMultiKW(this.rePlaceMulti, e, r, l, n, s, g)));
                    }
                  }
                }
              }
      ...
      for (var p = t.replace("___LIFTCHANGE___", this.rePlaceMulti_Done.join("")).replace(/<category[^>]*>(.*?)<\/category>/gi, "___STATELOCATION___"), u = 0; u < this.getStateAndLocation.location.length; u++) {
        var k = this.getStateAndLocation.location[u].trim();
        _ += '<category domain="category" nicename="' + buildStringURL(this.getStateAndLocation.location[u]) + '"><![CDATA[' + k + "]]></category>";
      }

      for (var d = 0; d < this.getStateAndLocation.state.length; d++) {
        var m = this.getStateAndLocation.state[d].trim();
        _ += '<category domain="category" nicename="' + buildStringURL(this.getStateAndLocation.state[d]) + '"><![CDATA[' + m + "]]></category>";
      }

      p = p.replace(/___STATELOCATION___/gi, _).replace(/___LIFTCHANGE___/gi, ""), $("#resultsmulti").val(unReplaceLIFT(p)), $("#results").val(LIFT_APP.KW.join("\n")), $("#number").text(this.rePlaceMulti_Done.length), $("#multiresults .uv").addClass("d-none"), $("#multiresults .rv").removeClass("d-none");
      var w = $("#resultsmulti").val(),
          v = "LIFT_POST_GEN_" + new Date().getTime() + ".xml";
      setTimeout(function () {
        download(v, w);
      }, 1e3);
    }
  },
  replaceMultiKW: function replaceMultiKW(t, e, _, i, r, a, l) {
    var c = _ ? _ + " " : "",
        n = i ? i + " " : "",
        o = r ? r + " " : "",
        s = a ? a + " " : "",
        h = l ? l + " " : "",
        g = _ || i || r || a || l ? c + n + o + s + h : "",
        p = makeid(5),
        u = t.match(/<item>(.*?)<\/item>/gi).map(function (t) {
      return t;
    })[0].replace(/(___REPLACE___|___replace___)/gi, g.trim()).replace(/(___REPLACE_A___|___replace_a___)/gi, c.trim()).replace(/(___REPLACE_B___|___replace_b___)/gi, n.trim()).replace(/(___REPLACE_C___|___replace_c___)/gi, o.trim()).replace(/(___REPLACE_D___|___replace_d___)/gi, s.trim()).replace(/(___REPLACE_E___|___replace_e___)/gi, h.trim()).trim();
    this.rePlaceMulti_Done.push(u.replace(/<wp:post_id>(.*?)<\/wp:post_id>/gi, "<wp:post_id>" + p + "</wp:post_id>"));
  }
};
LIFT_APP.init(), $("#l_c").change(function () {
  if (!$("#l_b").prop("checked")) return alert("Please lock B keyword first"), $(this).prop("checked", !1), !1;
}), $("#l_d").change(function () {
  if (!$("#l_c").prop("checked")) return alert("Please lock C keyword first"), $(this).prop("checked", !1), !1;
});
"use strict";

$("#checkHTML").on("click", function () {
  if (!$("#checkHTML_URL").val()) return alert("Please enter HTML Code"), !1;
}), $("#validator-btn").on("click", function () {
  var t,
      e = "",
      l = "";
  $("#htmlcode").val() || (e += "Please enter HTML Code\n"), e ? alert(e) : (t = $("#htmlcode").val().trim(), LIFT_APP.htmlResults_H1 = t.match(/<h1[^>]*>(.*?)<\/h1>/gi), LIFT_APP.htmlResults_H2 = t.match(/<h2[^>]*>(.*?)<\/h2>/gi), LIFT_APP.htmlResults_H3 = t.match(/<h3[^>]*>(.*?)<\/h3>/gi), LIFT_APP.htmlResults_H4 = t.match(/<h4[^>]*>(.*?)<\/h4>/gi), LIFT_APP.htmlResults_H5 = t.match(/<h5[^>]*>(.*?)<\/h5>/gi), LIFT_APP.htmlResults_H6 = t.match(/<h6[^>]*>(.*?)<\/h6>/gi), LIFT_APP.htmlResults_og_title = t.match(/og:title/gi), LIFT_APP.htmlResults_H1 ? (LIFT_APP.htmlResults_H1.map(function (t) {
    return t;
  }), 1 < LIFT_APP.htmlResults_H1.length && (l += "<li>We have more than 1 tag H1</li>")) : (l += "<li>We need improve H1 tag</li>", LIFT_APP.htmlResults_H1 = []), LIFT_APP.htmlResults_H2 ? LIFT_APP.htmlResults_H2 : LIFT_APP.htmlResults_H2 = [], LIFT_APP.htmlResults_H3 ? LIFT_APP.htmlResults_H3 : LIFT_APP.htmlResults_H3 = [], LIFT_APP.htmlResults_H4 ? LIFT_APP.htmlResults_H4 : LIFT_APP.htmlResults_H4 = [], LIFT_APP.htmlResults_H5 ? LIFT_APP.htmlResults_H5 : LIFT_APP.htmlResults_H5 = [], LIFT_APP.htmlResults_H6 ? LIFT_APP.htmlResults_H6 : LIFT_APP.htmlResults_H6 = [], l ? ($("#err_HTML").html('<ul class="mb-0">' + l + "</ul>").removeClass("d-none"), $("#done_HTML").addClass("d-none")) : ($("#err_HTML").addClass("d-none"), $("#done_HTML").removeClass("d-none")), $("#check_HTML").html('<div class="list-group">' + resultHTMLValidate("H1 Tag", LIFT_APP.htmlResults_H1.length) + resultHTMLValidate("H2 Tag", LIFT_APP.htmlResults_H1.length) + resultHTMLValidate("H3 Tag", LIFT_APP.htmlResults_H3.length) + resultHTMLValidate("H4 Tag", LIFT_APP.htmlResults_H1.length) + resultHTMLValidate("H5 Tag", LIFT_APP.htmlResults_H1.length) + resultHTMLValidate("H6 Tag", LIFT_APP.htmlResults_H1.length) + resultHTMLValidate("OG Title Tag", LIFT_APP.htmlResults_og_title ? "Ready" : "Undefined") + "</div>").removeClass("d-none"));
}), $("[data-preview-content]").each(function () {
  var s = $(this);
  $.ajax({
    url: "/previewURL?url=" + s.attr("data-preview-content"),
    cache: !0,
    dataType: "JSON",
    success: function success(t, e, l) {
      s.html('<div class="mb-3 row g-0 border rounded overflow-hidden flex-md-row shadow-sm h-md-250 position-relative" style="background: aliceblue;"> <div class="col p-4 d-flex flex-column position-static"> <div><strong class="d-inline-block mb-2 text-dark">' + extractRootDomain(s.attr("data-preview-content")) + '</strong><span class="ms-2"><a style="z-index: 1000; position: relative;" href="https://www.facebook.com/sharer.php?u=' + s.attr("data-preview-content") + "&t=" + t.title + '"><i class="fab fa-facebook-square"></i> Share</a></span></div> <h3 class="h4 text-primary mb-2">' + t.title + '</h3> <p class="card-text mb-auto mb-0">' + t.description + '</p>  <a href="' + s.attr("data-preview-content") + '" class="stretched-link" target="_blank">Open URL</a> </div> <div class="col-auto d-none d-md-block"> <div style="width:200px;min-height:100px;height:100%; background: ' + (10 < t.cover.length ? "" : "#dfdfdf") + " url(" + t.cover + ') center center no-repeat; background-size: cover"></div> </div> </div>'), 200 == l.status && (s.parents(".data-preview").find(".data-preview-url").remove(), 0 < $("#htmlcode").val().length && $("#validator-btn").trigger("click"));
    }
  });
}), $("#url").on("click", function () {
  var t = "";
  $("#urlsource").val() || (t += "Please enter URL\n"), t ? alert(t) : $.ajax({
    type: "GET",
...
$("#load").on("click", function () {
  LIFT_APP.__a_kw.val("Search Engine"), LIFT_APP.__b_kw.val("Optimization, Marketing"), LIFT_APP.__c_kw.val("Dallas, DFW"), LIFT_APP.__d_kw.val("tactics, strategy, strategies, blogs, blog, bloggers, blogger"), LIFT_APP.__e_kw.val("near me, here");
}), $("#generator").on("click", function () {
  LIFT_APP.__a_kw.val() && LIFT_APP.__b_kw.val() ? (LIFT_APP.KW = [], LIFT_APP.gen()) : alert("Please enter keywords");
}), $("#dwn-btn").on("click", function () {
  var e = $("#results").val(),
      t = "LIFT_KW_LIST_" + new Date().getTime() + ".txt";
  download(t, e);
}), $("#save-btn").on("click", function () {
  localStorage.removeItem("myLIFT"), localStorage.setItem("myLIFT", $("#results").val()), localStorage.removeItem("myLIFT_KW"), localStorage.setItem("myLIFT_KW", LIFT_APP.KW), $("#keywordds_toast").toast("show");
});
"use strict";

$("#clear").on("click", function () {
  localStorage.removeItem("myLIFT_KW"), localStorage.removeItem("myLIFT"), $("#keywordds").val(""), $("#keywordds_toast").toast("show");
}), $("#create-btn").on("click", function () {
  var e = "";
  if ($("#source").val() || (e += "Please upload source file\n"), $("#keywordds").val() || (e += "Please enter keywords list\n"), e) alert(e);else {
    for (var t = $("#keywordds").val().split("\n"), a = "", l = [], o = replaceLIFT(LIFT_APP.code.getValue()), r = o.replace(/<item>(.*?)<\/item>/gi, "___LIFTCHANGE___"), s = o.match(/<item>(.*?)<\/item>/gi).map(function (e) {
      return e;
    }), i = 0; i < s.length; i++) {
      a += "___LIFTCHANGE___";
    }
    ...
    for (var _ = 0; _ < t.length; _++) {
      var c = makeid(5);
      t[_] && 1 < t[_].length && l.push(s[0].replace(/(___REPLACE___|___replace___)/gi, t[_].trim()).replace(/<wp:post_id>(.*?)<\/wp:post_id>/gi, "<wp:post_id>" + c + "</wp:post_id>"));
    }

    var g = r.replace(a, l.join(""));
    $("#results").val(unReplaceLIFT(g));
    var k = $("#results").val(),
        d = "LIFT_KW_LIST_" + new Date().getTime() + ".xml";
    download(d, k);
  }
}), $("#keywordds").val(localStorage.getItem("myLIFT")), localStorage.getItem("keywordds_a") && $("#a_kw").val(localStorage.getItem("keywordds_a")), localStorage.getItem("keywordds_b") && $("#b_kw").val(localStorage.getItem("keywordds_b")), localStorage.getItem("keywordds_c") && $("#c_kw").val(localStorage.getItem("keywordds_c")), localStorage.getItem("keywordds_d") && $("#d_kw").val(localStorage.getItem("keywordds_d")), localStorage.getItem("keywordds_e") && $("#e_kw").val(localStorage.getItem("keywordds_e")), localStorage.getItem("keywordds_f") && $("#instate").val(localStorage.getItem("keywordds_f")), localStorage.getItem("keywordds_g") && $("#inlocation").val(localStorage.getItem("keywordds_g")), $(".btn-save").on("click", function () {
  localStorage.getItem($(this).attr("id")) ? (localStorage.removeItem($(this).attr("id")), $(this).parents(".relative").find("textarea").val(""), $(this).removeClass("ready").text("Save")) : 0 < $(this).parents(".relative").find("textarea").val().length ? (localStorage.removeItem($(this).attr("id")), localStorage.setItem($(this).attr("id"), $(this).parents(".relative").find("textarea").val()), $(this).addClass("ready").text("Clear")) : alert("Please enter keywords"), $("#keywordds_toast").toast("show");
}), $(".btn-save").each(function () {
  localStorage.getItem($(this).attr("id")) && $(this).addClass("ready").text("Clear");
});
"use strict";

$("#create-btnmulti").on("click", function () {
  LIFT_APP.__a_kw.val() && LIFT_APP.__b_kw.val() ? $("#source").val() ? (LIFT_APP.KW = [], LIFT_APP.genmulti()) : alert("Please upload file") : alert("Please enter keywords");
});
"use strict";

$("#contentgeneratorimport").on("click", function () {
  $("#totalurls").html(""), $("#contentresult").val("");
  var e = "";
  if ($("#csvsource").val() || LIFT_APP.code || (e += "Please upload .csv file\n"), e) alert(e);else {
    var l = null,
        t = "";
    $("#csvsource").val() && (l = $("#csvsource").val()), LIFT_APP.code && (l = LIFT_APP.code.getValue()), l = l.replace(/^(?:\r\n?|\n|\r|\s*)/gm, "").replace(/\n*$/, "").split("\n");

    for (var r = $("#readCSVTable #fkw").val(), a = $("#readCSVTable #furl").val(), n = 0; n < l.length; n++) {
      var c = "";
      0 < $("#add-title:checkbox:checked").length && (c += ' title="' + replaceLIFT(l[n].split(",")[r]).trim() + '"'), 0 < $("#add-nofollow:checkbox:checked").length && (c += ' rel="nofollow"'), 0 < $("#add-tab:checkbox:checked").length && (c += ' target="_blank"'), n < l.length - 1 ? t += '<a href="' + l[n].split(",")[a] + '"' + c + ">" + replaceLIFT(l[n].split(",")[r]).trim() + "</a>, " : t += '<a href="' + l[n].split(",")[a] + '"' + c + ">" + replaceLIFT(l[n].split(",")[r]).trim() + "</a>";
    }

    0 < t.length && ($("#boxresult .rv").hide(), $("#totalurls").html(t), $("#contentresult").val(t), $("#boxresult .rs").removeClass("d-none"));
  }
}), $("#contentgenerator").on("click", function () {
  $("#totalurls").html(""), $("#contentresult").val("");
  var e = "",
      l = $("#contentkeyword").val().replace(/^(?:\r\n?|\n|\r|\s*)/gm, "").replace(/\n*$/, "").split("\n"),
  ...
  if ($("#contentkeyword").val() || (e += "Please enter keywords list\n"), $("#contenturls").val() || (e += "Please enter URLs list\n"), l.length != t.length && (e += "Something wrong!\n"), e) alert(e);else {
    for (var r = "", a = 0; a < l.length; a++) {
      var n = "";
      0 < $("#add-title:checkbox:checked").length && (n += ' title="' + replaceLIFT(l[a]).trim() + '"'), 0 < $("#add-nofollow:checkbox:checked").length && (n += ' rel="nofollow"'), 0 < $("#add-tab:checkbox:checked").length && (n += ' target="_blank"'), a < l.length - 1 ? r += '<a href="' + t[a] + '"' + n + ">" + replaceLIFT(l[a]).trim() + "</a>, " : r += '<a href="' + t[a] + '"' + n + ">" + replaceLIFT(l[a]).trim() + "</a>";
    }

    0 < r.length && ($("#boxresult .rv").hide(), $("#totalurls").html(r), $("#contentresult").val(r), $("#boxresult .rs").removeClass("d-none"));
  }
});
var clipboard = new ClipboardJS("[data-clipboard-target]");
clipboard.on("success", function (e) {
  $("#copy_toast").toast("show"), e.clearSelection();
});
"use strict";

$("#robotCraw").on("click", function () {
  var n = "#robots.txt generator by LIFT Creations\n\n",
      e = ["/sitemap.xml"];
  if ($("#sAll").prop("checked")) n += "User-agent: *\n", "allow" === $("#inputAllow").val() ? n += "Allow: /\n\n" : n += "Disallow: /\n\n";else {
    if ($("#sRobots .bot :checkbox:checked").length <= 0) return alert("Please select User-agent"), !1;
    $("#sRobots").find(".bot :checkbox").each(function () {
      if ($(this).prop("checked")) {
        n += "# by " + $(this).attr("data-name") + "\n";
  ...
  0 < $("#inputDelay").val().length && (n += "Crawl-delay: " + $("#inputDelay").val() + "\n\n"), 0 < $("#inputSitemap").val().length && (e = $("#inputSitemap").val().replace(/^(?:\r\n?|\n|\r|\s*)/gm, "").replace(/\n*$/, "").split("\n"));
  ...
  $(this).on("click", function () {
    $(this).prop("checked") || $("#sAll").prop("checked", !1);
    var e = !0;
    $("#sRobots").find(".bot :checkbox").each(function () {
      $(this).prop("checked") || (e = !1);
    }), e && $("#sAll").prop("checked", !0);
  });
});
"use strict";
...
triggerTabList.forEach(function (t) {
  t.addEventListener("shown.bs.tab", function (t) {
    t.preventDefault();
    var i = $(t.target).attr("data-bs-target");
    $(i).find('input[type="text"], input[type="tel"], input[type="email"], textarea').val(""), $("#inputText").val(""), $("#inputAmount").val(""), $("#inputNetwork").val(""), $("#inputCall").val("");
  });
});
var img1 = "/qrcode?s=qr&sf=40&sy=40&wq=0&pl=0&pr=0&pt=0&ts=15&th=15&pb=0&d=",
    img2 = "/barcode?s=dmtx&sf=70&sy=70&wq=0&pl=0&pr=0&pt=0&ts=15&th=15&pb=0&d=",
    img3 = "/barcode?s=code-128&sf=12&sy=6&wq=0&pl=0&pr=0&pt=0&pb=0&ts=0&th=10&d=",
    img4 = "/barcode?s=code-39&sf=4&sy=1&wq=0&pl=0&pr=0&pt=0&ts=5&th=16&pb=18&d=";
$("#qrgenbtn").on("click", function () {
  var t, i, r, a, e;
  0 < $("#inputText").val().length && (t = $("#inputText").val().trim(), $("#imgqr-1").attr("src", img1 + t), $("#imgqr-2").attr("src", img2 + t), $("#imgqr-3").attr("src", img3 + t), $("#imgqr-4").attr("src", img4 + t)), 0 < $("#inputNetwork").val().length && (i = "WIFI:T:" + $('input[name="inlineRadioNWOptionsNW"]:checked', "#frmchange").val().trim() + ";S:" + $("#inputNetwork").val().trim() + ";P:" + $("#inputPassword").val().trim() + ";H:;", $("#imgqr-1").attr("src", img1 + i), $("#imgqr-2").attr("src", img2 + i), $("#imgqr-3").attr("src", img3 + i), $("#imgqr-4").attr("src", img4 + $("#inputNetwork").val().trim())), 0 < $("#inputAmount").val().length && (r = $('input[name="inlineRadioOptions"]:checked', "#frmchange").val().trim() + ":" + $("#inputReceiver").val().trim() + "?amount=" + $("#inputAmount").val().trim(), $("#imgqr-1").attr("src", img1 + r), $("#imgqr-2").attr("src", img2 + r), $("#imgqr-3").attr("src", img3 + r), $("#imgqr-4").attr("src", img4 + $('input[name="inlineRadioOptions"]:checked', "#frmchange").val().trim())), 0 < $("#inputCall").val().length && (a = "tel:" + $("#inputCall").val().trim(), $("#imgqr-1").attr("src", img1 + a), $("#imgqr-2").attr("src", img2 + a), $("#imgqr-3").attr("src", img3 + a), $("#imgqr-4").attr("src", img4 + $("#inputCall").val().trim()), console.log(a)), 0 < $("#inputFullname").val().length && (e = "BEGIN:VCARDVERSION:3.0N:" + $("#inputFullname").val().trim() + "FN:" + $("#inputFullname").val().trim() + "URL:" + $("#inputWebsite").val().trim() + "EMAIL:" + $("#inputEmail").val().trim() + "TEL;TYPE=voice,work,pref:" + $("#inputMobile").val().trim() + "ADR;TYPE=intl,work,postal,parcel:;;" + $("#inputAddress").val().trim() + "END:VCARD", $("#imgqr-1").attr("src", img1 + e), $("#imgqr-2").attr("src", img2 + e), $("#imgqr-3").attr("src", img3 + e), $("#imgqr-4").attr("src", img4 + $("#inputFullname").val().trim()), console.log(e));
}), $(".clicktosave").on("click", function () {
  var t = document.createElement("a");
  t.href = $(this).attr("src"), t.download = "QR-CODE-" + makeid(5) + ".jpg", document.body.appendChild(t), t.click();
});
"use strict";

function loadfilePreview() {
  $("#faviboxres .rv").removeClass("d-none"), $("#faviboxres .tv").addClass("d-none"), LIFT_APP.code && LIFT_APP.code.toTextArea();
  var e,
      n = '<link rel="apple-touch-icon" sizes="57x57" href="/favicon/apple-icon-57x57.png">\n<link rel="apple-touch-icon" sizes="60x60" href="/favicon/apple-icon-60x60.png">\n<link rel="apple-touch-icon" sizes="72x72" href="/favicon/apple-icon-72x72.png">\n<link rel="apple-touch-icon" sizes="76x76" href="/favicon/apple-icon-76x76.png">\n<link rel="apple-touch-icon" sizes="114x114" href="/favicon/apple-icon-114x114.png">\n<link rel="apple-touch-icon" sizes="120x120" href="/favicon/apple-icon-120x120.png">\n<link rel="apple-touch-icon" sizes="144x144" href="/favicon/apple-icon-144x144.png">\n<link rel="apple-touch-icon" sizes="152x152" href="/favicon/apple-icon-152x152.png">\n<link rel="apple-touch-icon" sizes="180x180" href="/favicon/apple-icon-180x180.png">\n<link rel="icon" type="image/png" sizes="192x192"  href="/favicon/android-icon-192x192.png">\n<link rel="icon" type="image/png" sizes="32x32" href="/favicon/favicon-32x32.png">\n<link rel="icon" type="image/png" sizes="96x96" href="/favicon/favicon-96x96.png">\n<link rel="icon" type="image/png" sizes="16x16" href="/favicon/favicon-16x16.png">\n<link rel="manifest" href="/favicon/manifest.json">\n<meta name="mobile-web-app-capable" content="yes">\n<meta name="apple-mobile-web-app-title" content="' + $("#sitename").val().trim() + '">\n<meta name="apple-mobile-web-app-capable" content="yes">\n<meta name="apple-touch-fullscreen" content="yes">\n<meta name="apple-mobile-web-app-status-bar-style" content="black">\n<link rel="shortcut icon" type="image/ico" href="/favicon/favicon.ico">\n<link rel="fluid-icon" type="image/png" href="/favicon/apple-icon.png">\n<meta name="msapplication-config" content="/favicon/browserconfig.xml">\n<meta name="msapplication-TileColor" content="' + $("#sitecolor").val().trim() + '">\n<meta name="msapplication-TileImage" content="/favicon/ms-icon-144x144.png">\n<meta name="theme-color" content="' + $("#sitecolor").val().trim() + '">',
      i = $("#filePreview").get(0).files[0];
  i ? ((e = new FileReader()).onload = function (e) {
    $("#favicon-preview .faicon").css({
      "background-image": "url(" + e.target.result + ")"
    }), $("#favicon-preview-icon .imgfavi").attr("src", e.target.result);
  }, e.readAsDataURL(i), $("#favisresult").val(n)) : alert("Please upload favicon");
}

$("#sitecolor").on("change", function () {
  loadfilePreview();
}), $("#getfrmSubmit").on("submit", function () {
  if (!$("#filePreview").get(0).files[0]) return alert("Please upload favicon"), !1;
});
"use strict";

0 < $("#geostate").length && $.ajax({
  ...
  success: function success(e) {
    ...
    for (var n in e) {
      Object.hasOwnProperty.call(e, n) && (t += '<option value="' + n + '">' + e[n] + "</option>");
    }
  ...
  LIFT_APP.code && LIFT_APP.code.toTextArea();
  var n = $("#geoaddress").val().trim(),
      a = $("#geowebname").val().trim(),
      o = $("#geoemail").val().trim(),
      m = $("#geophone").val().trim(),
      r = $("#geofax").val().trim(),
      s = $("#geostate").val().trim();
  if (!n || n.length < 1) return alert("Please enter address"), !1;
  $.ajax({
    url: "https://nominatim.openstreetmap.org/search?q=" + encodeURIComponent(n) + "&format=json&json_callback=&addressdetails=1&limit=1",
    type: "GET",
    success: function success(e) {
      var t = '\x3c!--//\n======================================\n╦  ╦╔═╗╔╦╗  ╔═╗┬─┐┌─┐┌─┐┌┬┐┬┌─┐┌┐┌┌─┐\n║  ║╠╣  ║   ║  ├┬┘├┤ ├─┤ │ ││ ││││└─┐\n╩═╝╩╚   ╩   ╚═╝┴└─└─┘┴ ┴ ┴ ┴└─┘┘└┘└─┘\n======================================\n//--\x3e\n<meta name="DC.title" content="' + a + '" />\n<meta name="geo.region" content="' + e[0].address.country_code.toUpperCase() + "-" + s + '" />\n<meta name="geo.placename" content="' + e[0].address.city + '" />\n<meta name="geo.position" content="' + e[0].lat + ";" + e[0].lon + '" />\n<meta name="address" content="' + e[0].display_name + '" />\n<meta name="ICBM" content="' + e[0].lat + ", " + e[0].lon + '" />\n<meta name="og:latitude" content="' + e[0].lat + '" />\n<meta name="og:longitude" content="' + e[0].lon + '" />\n<meta name="og:street-address" content="' + n + '" />\n<meta name="og:locality" content="' + e[0].address.city + '" />\n<meta name="og:region" content="' + s + '" />\n<meta name="og:postal-code" content="' + e[0].address.postcode + '" />\n<meta name="og:email" content="' + o + '" />\n<meta name="og:phone_number" content="' + m + '" />\n<meta name="og:fax_number" content="' + r + '" />\n<meta name="og:country-name" content="USA" />';
      $("#geosource").val(t), LIFT_APP.code = CodeMirror.fromTextArea(document.getElementById("geosource"), {
        mode: "text/html",
        lineNumbers: !0,
        styleActiveLine: !0,
        matchBrackets: !0,
