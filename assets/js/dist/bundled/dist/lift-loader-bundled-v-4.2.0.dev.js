...
function makeid(e) {
  for (var t = [], n = "123456789", o = n.length, a = 0; a < e; a++) {
...
function loadCSVFileAsText() {
  ...
  document.getElementById("fileCSVToLoad").value ? (e = document.getElementById("fileCSVToLoad").files[0], t = new FileReader(), LIFT_APP.code && LIFT_APP.code.toTextArea(), t.onload = function (e) {
    ...
    for (var n = LIFT_APP.code.getValue().replace(/^(?:\r\n?|\n|\r|\s*)/gm, "").replace(/\n*$/, "").split("\n")[0].split(","), o = "", a = 0; a < n.length; a++) {
      o += '<option value="' + a + '">' + n[a] + "</option>";
    }
...
var LIFT_APP = {
  ...
  gen: function gen() {
    for (var t = 0; t < this.akw_get().length; t++) {
      ...
      if (0 < e.length) for (var i = 0; i < this.bkw_get().length; i++) {
        var _ = lift_encode(this.bkw_get()[i].trim());

        if (0 < _.length) {
          this.KW.push(e + " " + _);

          for (var n = 0; n < this.ckw_get().length; n++) {
            var r = lift_encode(this.ckw_get()[n].trim());

            if (0 < r.length) {
              this.KW.push(e + " " + _ + " " + r);

              for (var s = 0; s < this.dkw_get().length; s++) {
                var l = lift_encode(this.dkw_get()[s].trim());

                if (0 < l.length) {
                  this.KW.push(e + " " + _ + " " + r + " " + l);

                  for (var h = 0; h < this.ekw_get().length; h++) {
                    var k = lift_encode(this.ekw_get()[h].trim());
                    0 < k.length && this.KW.push(e + " " + _ + " " + r + " " + l + " " + k);
                  }
                }
              }
            }
...
$("#load").on("click", function () {
  ...
  localStorage.removeItem("myLIFT_A"), localStorage.setItem("myLIFT_A", $("#a_kw").val()), localStorage.removeItem("myLIFT_B"), localStorage.setItem("myLIFT_B", $("#b_kw").val()), localStorage.removeItem("myLIFT_C"), localStorage.setItem("myLIFT_C", $("#c_kw").val()), localStorage.removeItem("myLIFT_D"), localStorage.setItem("myLIFT_D", $("#d_kw").val()), localStorage.removeItem("myLIFT_E"), localStorage.setItem("myLIFT_E", $("#e_kw").val()), localStorage.removeItem("myLIFT"), localStorage.setItem("myLIFT", $("#results").val()), localStorage.removeItem("myLIFT_KW"), localStorage.setItem("myLIFT_KW", LIFT_APP.KW), $("#keywordds_toast").toast("show");
});
"use strict";
...
$("#clear").on("click", function () {
  ...
  localStorage.removeItem("myLIFT_A"), $("#keywordds_a").val(""), $("#keywordds_toast").toast("show");
}), $("#clear_b").on("click", function () {
  localStorage.removeItem("myLIFT_B"), $("#keywordds_b").val(""), $("#keywordds_toast").toast("show");
}), $("#clear_c").on("click", function () {
  localStorage.removeItem("myLIFT_C"), $("#keywordds_c").val(""), $("#keywordds_toast").toast("show");
}), $("#clear_d").on("click", function () {
  localStorage.removeItem("myLIFT_D"), $("#keywordds_d").val(""), $("#keywordds_toast").toast("show");
}), $("#clear_e").on("click", function () {
  localStorage.removeItem("myLIFT_E"), $("#keywordds_e").val(""), $("#keywordds_toast").toast("show");
}), $("#create-btn").on("click", function () {
  var e = "";
  if ($("#source").val() || (e += "Please upload source file\n"), $("#keywordds").val() || (e += "Please enter keywords list\n"), e) alert(e);else {
    for (var o = $("#keywordds").val().split("\n"), t = "", a = [], l = replaceLIFT(LIFT_APP.code.getValue()), r = l.replace(/<item>(.*?)<\/item>/gi, "___LIFTCHANGE___"), _ = l.match(/<item>(.*?)<\/item>/gi).map(function (e) {
      return e;
    }), s = 0; s < _.length; s++) {
      t += "___LIFTCHANGE___";
    }
    ...
    for (var c = 0; c < o.length; c++) {
      var k = makeid(5);
      o[c] && 1 < o[c].length && a.push(_[0].replace(/___REPLACE___/gi, o[c].trim()).replace(/<wp:post_id>(.*?)<\/wp:post_id>/gi, "<wp:post_id>" + k + "</wp:post_id>"));
    }

    var m = r.replace(t, a.join(""));
    $("#results").val(unReplaceLIFT(m));
    var i = $("#results").val(),
        n = "LIFT_KW_LIST_" + new Date().getTime() + ".xml";
    download(n, i);
  }
}), $("#keywordds").val(localStorage.getItem("myLIFT")), $("#keywordds_a").val(localStorage.getItem("myLIFT_A")), $("#keywordds_b").val(localStorage.getItem("myLIFT_B")), $("#keywordds_c").val(localStorage.getItem("myLIFT_C")), $("#keywordds_d").val(localStorage.getItem("myLIFT_D")), $("#keywordds_e").val(localStorage.getItem("myLIFT_E"));
"use strict";
...
var img1 = "/qrcode.php?s=qr&sf=40&sy=40&wq=0&pl=0&pr=0&pt=0&ts=15&th=15&pb=0&d=",
    img2 = "/barcode.php?s=dmtx&sf=70&sy=70&wq=0&pl=0&pr=0&pt=0&ts=15&th=15&pb=0&d=",
    img3 = "/barcode.php?s=code-128&sf=12&sy=6&wq=0&pl=0&pr=0&pt=0&pb=0&ts=0&th=10&d=",
    img4 = "/barcode.php?s=code-39&sf=4&sy=1&wq=0&pl=0&pr=0&pt=0&ts=5&th=16&pb=18&d=";
