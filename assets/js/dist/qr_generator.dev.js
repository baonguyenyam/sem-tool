...
triggerTabList.forEach(function (triggerEl) {
  triggerEl.addEventListener('shown.bs.tab', function (event) {
    ...
    var dm = $(event.target).attr('data-bs-target');
...
$('#qrgenbtn').on('click', function () {
  ...
  if ($('#inputNetwork').val().length > 0) {
    var vWifi = 'WIFI:' + 'T:' + $('input[name="inlineRadioNWOptionsNW"]:checked', '#frmchange').val().trim() + ';' + 'S:' + $('#inputNetwork').val().trim() + ';' + 'P:' + $('#inputPassword').val().trim() + ';' + 'H:;';
  ...
  if ($('#inputAmount').val().length > 0) {
    var vBit = '' + $('input[name="inlineRadioOptions"]:checked', '#frmchange').val().trim() + ':' + '' + $('#inputReceiver').val().trim() + '' + '?amount=' + $('#inputAmount').val().trim();
  ...
  if ($('#inputCall').val().length > 0) {
    var vTel = 'tel:' + '' + $('#inputCall').val().trim();
  ...
  if ($('#inputFullname').val().length > 0) {
    var vCard = 'BEGIN:VCARD' + 'VERSION:3.0' + 'N:' + $('#inputFullname').val().trim() + 'FN:' + $('#inputFullname').val().trim() + 'URL:' + $('#inputWebsite').val().trim() + 'EMAIL:' + $('#inputEmail').val().trim() + 'TEL;TYPE=voice,work,pref:' + $('#inputMobile').val().trim() + 'ADR;TYPE=intl,work,postal,parcel:;;' + $('#inputAddress').val().trim() + 'END:VCARD';
