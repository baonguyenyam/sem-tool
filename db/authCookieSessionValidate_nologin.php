...
if (!empty($_SESSION["member_id"])) {
    $isLoggedIn = true;
    $isMemberTypye = $auth->getMemberByID($_SESSION["member_id"])[0]["member_type"];
    if ($auth->getLogin($_SESSION["member_id"]) && count($auth->getLogin($_SESSION["member_id"])) > 0) {
        $auth->updateLogin(time(), $_SESSION["member_id"]);
    } else {
        $auth->insertLogin(time(), $_SESSION["member_id"]);
    }
